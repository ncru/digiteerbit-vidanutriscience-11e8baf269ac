class ShipmentValidatePiece
  ...
  def initialize(options = {})
    ...
    if options[:weight] && options[:weight] > 0
      ...
      raise ShipmentValidate::OptionsError, required_option_error_message(:weight)
    end
    ...
    if options[:width] || options[:height] || options[:depth]
      [ :width, :height, :depth ].each do |req|
        if options[req].to_f > 0.0
          ...
          raise ShipmentValidate::OptionsError, required_option_error_message(req)
        end
      end
    end
...
private
  ...
  def required_option_error_message(field)
    ":#{field} is a required for ShipmentValidatePiece. Must be nonzero integer or float."
  end
end
